...
const ReductionDay: React.FC<Props>  = ({timeUnit, dayDeviceTypeColor, server_id='a00000000000000'}) => {
	...
	let [deviceType] = useState(['综合设备', '照明设备', '空调设备']); // 默认的设备类型
	let [dayDeviceTypeName, setDayDeviceTypeName]: any[] = useState([]);
	...
	const getDayDemand = async () => {
		await api.GetDayDemand().then((res: any)=>{
			setDayDemandList(res)
		})
	}
	// 供给端
	const getDaySupply = async () => {
		await api.GetDaySupply().then((res: any)=>{
			setDaySupplyList(res)
		})
	}
	// 环境
	const getEnvironment = async () => {
		await reductionApi.GetEnvironment().then((res: any)=>{
			setEnvironment(res)
		})
	}
	// 获取设备类型
	...
	// 柱形图
	const getDayStrategy = async (server_id: any, type: any, device: any) => {
		let params = {
			day_str: getNowDate(),
			server_id,
			type,
		...
		await api.GetDayStrategy(params).then((res: any)=>{
			let arr = res.data;
			setUnit(res.unit)
			if (type === 'comparison') {
				arr.forEach((el: any) => {
	...
	return (
		<ReductionLayout>
			<div className="day-page w-100 h-100">
				<Title title="日策略" color={'white'} showHelp={true} helpImgUrl={helpImgUrl} showSelect={true}/>
