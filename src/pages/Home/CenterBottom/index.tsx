...
function CenterBottom(props: Props) {
	let {node, selectDate, timeUnit} = props;
	...
	// 接口获取数据
	const getPredict = async function(
		date: string,
		selectValue: string,
		dataType: any,
		unit: string,
		...
		const res: any = await api.GetPredict(params);
		let obj: {prediction?: number, reality?: number} = {};
		res.forEach((el: any) => {
			if(el.name === '预测值') {
	...
	const ifHaveIntersectionPoint = function(a1: any, b1: any, a2: any, b2: any) {
		return (+a1 > +b1) !== (+a2 > +b2);
	}

	// 是否执行后续的计算 ？ 不是最后一个点，且有交点时
	const ifCalculatePoint = function(idx: any, lth: any, [a1, b1, a2, b2]: [any, any, any, any]) {
