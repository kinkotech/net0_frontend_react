...
const BottomLeft: React.FC<Props> = function ({start, end, park_id}) {
    let [unit, setUnit] = useState('');
    let [option, setOption] = useState({});
    let [number, setNumber] = useState(0);
    let [sum, setSum] = useState(0);
    let [max, setMax] = useState(0);
    console.log(unit, number, sum, max)
    ...
    // 获取数据
    const getActivityComparison = async (start: any, end: any, park_id: any) => {
        let params = {
            park_id,
            start,
            end,
        }
        await api.GetActivityComparison(params).then((res: any)=>{
            let result = res;
            let key = result.key;
            let data: any[] = [];
            ...
            result.value.forEach((el: any, i: number) => {
                data.push({
                    value: el,
                    name: key[i]
                })
    ...
    return (
        <div className="bottom-left d-flex flex-column w-100 h-100">
            <ConTitle title="范围类别排放量占比" showPopver={true} popverContent="popverContent"/>
