...
const Right: React.FC<Props> = ({start, end, park_id}) => {
    console.log(start,end,'000')
    let [rightTopValue, setRightTopValue]: any = useState({});
    let [rightBottomValue, setRightBottomValue]: any = useState([]);
    let [rightCenterValue, setRightCenterValue]: any = useState({});
    let [trendUp, setTrendUp]: any = useState(null);
    let [currentYear]: any = useState('');
    let [preStart, setPreStart]: any = useState();
    let [preEnd, setPreEnd]: any = useState();
    ...
    const getCycleComparison = async (start: any, end: any) => {
        let params = {
            park_id,
            start,
            end
        }
        await api.GetCycleComparison(params).then((res: any)=>{
    ...
    // 零碳园区能源系统指标
    const getEnergyTarget = async (start: any, end: any) => {
        let params = {
            park_id,
            start,
            end
        }
        await api.GetEnergyTarget(params).then((res: any)=>{
            setRightBottomValue(res)
        })
    }
    
    // 绿色能源占比
    const getGreenEnergy = async (start: any, end: any) => {
        let params = {
            park_id,
            start,
            end
            ...
            <div className="right-bottom h-100">
                ...
                <div className="right-bottom-center">
                    ...
                    <div className="right-bottom-center-con h-100">
                        {
                            rightBottomValue.map((item: any, i: number) => {
                                return (
